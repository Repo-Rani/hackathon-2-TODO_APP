<!-- SYNC IMPACT REPORT
Version: 2.0.0
Note: This is a new constitution file for "Phase IV - Local Kubernetes Deployment". The original constitution for Phase I remains at .specify/memory/constitution.md.
Templates requiring updates:
- README.md: ⚠ pending
Follow-up TODOs:
- The README.md file is significantly out of date and should be updated to reflect the new Phase IV constitution.
- The project now has multiple constitution files. A decision should be made on which one is active or how they should be consolidated.
-->
# Constitution: Phase IV - Local Kubernetes Deployment
...
- **Health Checks**: Every container MUST expose a `/health` endpoint.
...
```bash
# Preferred: AI-assisted Docker operations
...
```bash
# Preferred: AI-assisted Kubernetes operations
...
```bash
# Advanced cluster analysis and optimization
...
```bash
# All pods MUST reach Running state within 2 minutes
...
- [x] Todo Chatbot from Phase III fully operational in Kubernetes
- [x] Frontend accessible via Minikube IP or ingress
- [x] Backend MCP server responding to agent requests
- [x] Database connection to Neon working from pods
- [x] Better Auth authentication functioning in containerized environment
- [x] All Basic Level features (Add, Delete, Update, View, Mark Complete) working

### 9.3 Infrastructure Requirements
- [x] Dockerfiles using multi-stage builds
- [x] Kubernetes deployments with health checks
- [x] Helm chart with configurable values
- [x] Secrets properly managed (not in code)
- [x] Services configured for inter-pod communication
- [x] Ingress or NodePort for external access

### 9.4 AI DevOps Requirements
- [x] Evidence of Gordon usage in Docker operations (screenshots/logs)
- [x] Evidence of `kubectl-ai` usage in K8s operations
- [x] Evidence of Kagent usage for cluster analysis
- [x] All commands documented in deployment runbook
...
2. Deploying with Helm (or `kubectl-ai`).
3. Accessing Todo Chatbot in browser.
4. Adding a task via natural language.
5. Checking pod status with `kubectl`.
6. (Bonus) Using Gordon/`kubectl-ai`/Kagent.
...
| ------------------------- | ------ | ----------------------------------------------------------- |
| Spec-Driven Compliance    | 25%    | Evidence of `Specify -> Plan -> Tasks -> Implement` cycle   |
| Container Quality         | 20%    | Dockerfile optimization, security, multi-stage builds       |
| Kubernetes Architecture   | 25%    | Proper resource definitions, health checks, services        |
| Helm Chart Usability      | 15%    | Clean templates, configurable values, documentation         |
| AIOps Tool Usage          | 10%    | Effective use of Gordon, `kubectl-ai`, Kagent                |
| Documentation             | 5%     | README, diagrams, troubleshooting guide                     |
...
1. **Check the specs first** - The answer is usually in your specification files.
2. **Ask `kubectl-ai`/Kagent** - "check why pods are failing".
3. **Consult `AGENTS.md`** - Ensure you're following the prescribed workflow.
4. **Use Claude Code** - Describe the problem, reference the spec, let it solve.
